...
export async function generarPdfSolicitud(solicitud) {
  ...
  // ---------------- Firma (anclada a la parte inferior de la página) ----------------
  const FIRMA_LINEA_Y = 108;
  const FIRMA_LABEL_Y = 92;
  if (state.y - 18 < FIRMA_LINEA_Y) {
    // Si de verdad no queda espacio en esta página, se ancla en una nueva —
    // pero en el caso normal (contenido de 1 página) se queda en la misma.
    nuevaPagina();
  }
  const lineaX1 = MARGIN + 40;
  const lineaX2 = A4_WIDTH - MARGIN - 40;
  state.page.drawLine({
    start: { x: lineaX1, y: FIRMA_LINEA_Y },
    end: { x: lineaX2, y: FIRMA_LINEA_Y },
    thickness: 0.8,
    color: TEXT,
  });
