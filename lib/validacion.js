...
// Opciones válidas para los campos nuevos del paso 1
...
export const ANTECEDENTES_ACADEMICOS = [
  "Profesor General Básica",
  "Profesor otras asignaturas",
];
...
export function validaSolicitud(body) {
  ...
  const actividadesVicaria = (body.actividadesVicaria || "").trim();
  const antecedentesAcademicos = (body.antecedentesAcademicos || "").trim();

  const cantidadHoras = Number.parseInt(body.cantidadHoras, 10);
  const nivelesEducacion = (
    Array.isArray(body.nivelesEducacion) ? body.nivelesEducacion : []
  )
    .map((n) => String(n).trim())
    .filter((n) => NIVELES_EDUCACION.includes(n));

  if (!Number.isInteger(cantidadHoras) || !HORAS_OPCIONES.includes(cantidadHoras)) {
    errores.cantidadHoras = "Selecciona la cantidad de horas (de 2 en 2, hasta 40).";
  }

  if (nivelesEducacion.length === 0) {
    errores.nivelesEducacion = "Selecciona al menos un nivel de educación.";
  }

  if (!ANTECEDENTES_ACADEMICOS.includes(antecedentesAcademicos)) {
    errores.antecedentesAcademicos = "Selecciona tus antecedentes académicos.";
  }
  ...
  data.cantidadHoras = Number.isInteger(cantidadHoras) ? cantidadHoras : null;
  data.nivelesEducacion = nivelesEducacion;
  data.antecedentesAcademicos = antecedentesAcademicos;
